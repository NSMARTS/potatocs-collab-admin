...
export class EditEmployeeInfoComponent implements OnInit {
	...
	updateInfo() {
		...
		this.dialogService.openDialogConfirm('Do you want to change employee information?').subscribe(result => {
			if (result) {
				let employeeInfo;
				...
				this.employeeMngmtService.putEmployeeInfo(employeeInfo).subscribe(
					(data: any) => {
						...
						if (data.message == 'updated') {
							this.router.navigate(['leave/employee-mngmt/manager-list']);
							this.dialogService.openDialogPositive('succeed employee info edit!');
						}
					},
					err => {
						console.log(err);
